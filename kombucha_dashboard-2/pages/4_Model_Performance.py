...
from kombucha import models, storage, theme
...
st.caption("Trained and validated on logged readings only -- live-only anomaly readings are never stored, so they can't leak into this.")

DEFAULT_BOUNDS_F1 = {"pH": (3.0, 4.3), "conductivity": (200, 1400)}
DEFAULT_BOUNDS_F2 = {"pressure": (0.0, 3.5)}
...
if not sim.empty and not real.empty:
    sim["batch"] = sim["batch"] + int(real["batch"].max()) + 1
...
if process == "F1":
    ...
    fpi_days_pred = combined.apply(
        lambda r: f1_fpi(
            r["pH"], r["conductivity"], r["temperature_C"], DEFAULT_BOUNDS_F1
        )["days_remaining"],
        axis=1,
    )
else:
    ...
    fpi_days_pred = combined.apply(
        lambda r: f2_fpi(
            r["pressure_bar"], r["temperature_C"], DEFAULT_BOUNDS_F2
        )["days_remaining"],
        axis=1,
    )
...
comp = pd.DataFrame(
    {
        "RMSE (days)": [fpi_rmse, overall["rmse_ridge"], overall["rmse_mixed"]],
        "MAE (days)": [fpi_mae, overall["mae"], np.nan], # (MAE mostly mapped for Ridge in this view)
        "R2": [fpi_r2, overall["r2"], np.nan],
    },
    index=["Hand-built FPI formula", "Tuned RidgeCV", "Mixed-Effects Model (RMSE only)"],
)
