...
def f1_alerts(latest, batch_day):
    """
    latest: dict/Series with pH, conductivity, temperature_C
    ...
    if temp < config.F1_TOO_COLD_C:
        alerts.append((
            "warning",
            f"Temperature {temp:.1f}C is below the ideal range; fermentation may be slowed."
        ))
    elif temp > config.F1_TOO_HOT_C:
        alerts.append((
            "danger",
            f"Temperature {temp:.1f}C is above the ideal range; risk of off-flavors or a stressed culture."
        ))
...
def f2_alerts(latest, batch_day, pressure_history):
    ...
    if temp < ideal - 3:
        alerts.append((
            "warning",
            f"Temperature {temp:.1f}C is low; carbonation will build slowly."
        ))
    elif temp > ideal + 3:
        alerts.append((
            "danger",
            f"Temperature {temp:.1f}C is high; risk of overpressure and off-flavors."
        ))
    ...
    if batch_day > 3 and latest["pressure_bar"] < config.F2_PRESSURE_STALL_BAR:
        alerts.append((
            "warning",
            f"Pressure is only {latest['pressure_bar']:.2f} bar after {batch_day:.1f} days; possible low yeast activity."
        ))
