...
def f1_fpi(pH, conductivity, turbidity, color, temp_c, bounds):
    """
    bounds: dict with (min, max) for each sensor, e.g.
        {"pH": (3.0, 4.3), "conductivity": (200, 1400),
         "turbidity": (5, 120), "color": (0, 100)}
    ...
    turb_n = normalize(turbidity, *bounds["turbidity"], invert=True)
    color_n = normalize(color, *bounds["color"], invert=False)

    raw_fpi = (
        config.F1_WEIGHTS["pH"] * pH_n
        + config.F1_WEIGHTS["conductivity"] * cond_n
        + config.F1_WEIGHTS["turbidity"] * turb_n
        + config.F1_WEIGHTS["color"] * color_n
    )
    ...
    return {
        "pH_n": pH_n, "cond_n": cond_n, "turb_n": turb_n, "color_n": color_n,
        "raw_fpi": raw_fpi, "temp_eff": t_eff, "temp_factor": t_factor,
        "fpi": fpi, "temp_adjust": t_adjust,
        "days_remaining": days_remaining,
        "progress_pct": max(0.0, min(100.0, progress_pct)),
    }


def f2_fpi(pressure, water_level, temp_c, bounds):
    """
    bounds: dict with (min, max) for pressure and water_level, e.g.
        {"pressure": (0.0, 3.5), "water_level": (60, 100)}
    """
    pressure_n = normalize(pressure, *bounds["pressure"], invert=False)
    water_n = normalize(water_level, *bounds["water_level"], invert=False)

    raw_fpi = (
        config.F2_WEIGHTS["pressure"] * pressure_n
        + config.F2_WEIGHTS["water_level"] * water_n
    )
    ...
    t_adjust = temp_adjust_multiplier(
        temp_c, config.F2_IDEAL_TEMP_C,
        config.F2_IDEAL_TEMP_C - 3, config.F2_IDEAL_TEMP_C + 3,
    )
    days_remaining = max(0.0, config.F2_D_MAX_DAYS * (1 - fpi) * t_adjust)
    progress_pct = 100.0 * (1 - days_remaining / config.F2_D_MAX_DAYS)

    return {
        "pressure_n": pressure_n, "water_n": water_n,
        "raw_fpi": raw_fpi, "temp_eff": t_eff, "temp_factor": t_factor,
        "fpi": fpi, "temp_adjust": t_adjust,
        "days_remaining": days_remaining,
        "progress_pct": max(0.0, min(100.0, progress_pct)),
