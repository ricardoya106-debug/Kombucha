...
e.g. for F1:  4.02,410.5,88.2,22.1,26.8,97.4
...
F1_FIELDS = ["pH", "conductivity", "turbidity", "color", "temperature_C", "water_level_pct"]
F2_FIELDS = ["pressure_bar", "temperature_C", "water_level_pct"]


def read_live(port, baud_rate, fields, timeout=2.0):
...
def read_mock(process="F1", state=None):
    ...
    if process == "F1":
        prev = state or {
            "pH": 4.1, "conductivity": 280, "turbidity": 100,
            "color": 12, "temperature_C": 27, "water_level_pct": 98,
        }
        reading = {
            "pH": max(3.0, prev["pH"] - random.uniform(0, 0.01)),
            "conductivity": prev["conductivity"] + random.uniform(0, 6),
            "turbidity": max(5, prev["turbidity"] - random.uniform(0, 0.5)),
            "color": prev["color"] + random.uniform(0, 0.3),
            "temperature_C": 27 + random.uniform(-1, 1),
            "water_level_pct": max(80, prev["water_level_pct"] - random.uniform(0, 0.05)),
        }
    else:
        prev = state or {"pressure_bar": 0.1, "temperature_C": 22, "water_level_pct": 95}
        reading = {
            "pressure_bar": prev["pressure_bar"] + random.uniform(0, 0.03),
            "temperature_C": 22 + random.uniform(-1, 1),
            "water_level_pct": max(80, prev["water_level_pct"] - random.uniform(0, 0.03)),
        }
    return reading
