...
F1_FEATURES = ["pH_n", "cond_n", "turb_n", "color_n", "temp_eff"]
F2_FEATURES = ["pressure_n", "water_n", "temp_eff"]
...
def build_feature_frame_f1(raw_df, bounds):
    ...
    turb_n = raw_df["turbidity"].apply(lambda v: normalize(v, *bounds["turbidity"], invert=True))
    color_n = raw_df["color"].apply(lambda v: normalize(v, *bounds["color"], invert=False))
    temp_eff = raw_df["temperature_C"].apply(
        lambda v: temperature_factor(v, config.F1_IDEAL_TEMP_C, config.F1_TEMP_SIGMA)[1]
    )
    feat = pd.DataFrame({
        "pH_n": pH_n, "cond_n": cond_n, "turb_n": turb_n,
        "color_n": color_n, "temp_eff": temp_eff,
    })
...
def build_feature_frame_f2(raw_df, bounds):
    ...
    water_n = raw_df["water_level_pct"].apply(lambda v: normalize(v, *bounds["water_level"], invert=False))
    temp_eff = raw_df["temperature_C"].apply(
        lambda v: temperature_factor(v, config.F2_IDEAL_TEMP_C, config.F2_TEMP_SIGMA)[1]
    )
    feat = pd.DataFrame({"pressure_n": pressure_n, "water_n": water_n, "temp_eff": temp_eff})
    feat["batch"] = raw_df["batch"].values
    feat["actual_days_remaining"] = raw_df["actual_days_remaining"].values
    return feat
