...
if autorefresh_available:
    st_autorefresh(interval=60_000, key="f1_autorefresh")  # tick every minute; the time-gate below decides when to actually log

manual_log = st.sidebar.button("Log a reading now")

# ---------------- Time-gated logging ----------------
if "f1_last_log_time" not in st.session_state:
    st.session_state.f1_last_log_time = None

now = dt.datetime.now()
auto_due = autorefresh_available and (
    st.session_state.f1_last_log_time is None
    or (now - st.session_state.f1_last_log_time).total_seconds() >= interval_min * 60
)
log_now = manual_log or auto_due

if log_now:
    if use_sim:
        reading = serial_reader.read_mock("F1", st.session_state.get("f1_mock_state"))
        st.session_state.f1_mock_state = reading
    else:
        reading = None
        try:
            reading = serial_reader.read_live(port, config.SERIAL_BAUD_RATE, serial_reader.F1_FIELDS)
...
bounds = DEFAULT_BOUNDS  # replace with bounds fit to your own historical batches once you have several

result = f1_fpi(latest["pH"], latest["conductivity"], latest["turbidity"], latest["color"], latest["temperature_C"], bounds)
...
# ---------------- Alerts ----------------
st.subheader("Status")
batch_alerts = alerts.f1_alerts(latest, day_elapsed)
if batch_alerts:
...
sensor_cols = ["pH", "conductivity", "turbidity", "color", "temperature_C", "water_level_pct"]
tabs = st.tabs(sensor_cols)
for tab, col in zip(tabs, sensor_cols):
    with tab:
        fig = px.line(hist, x="day", y=col, markers=True, title=f"{col} vs. day")
